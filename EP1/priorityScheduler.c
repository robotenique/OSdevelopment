...
static double *quantum; // array with the priority of each process
...
double applyLogSigmoid(double);
double calcQuanta(double);
void *runPScheduler(void *);
double calculatePriority(Process p);
void addToStats(double priority);
void removeFromStats(double);
...
void schedulerPriority(ProcArray pQueue){
    ...
    quantum = emalloc(sizeof(double)*sz); // Array with the priority of each process
 ...
 *
 * @args args : the node of the process
 *
 * @return
 */
